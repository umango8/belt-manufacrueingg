...
export default function Preloader() {
  ...
  // Determine branding details dynamically based on path
  const isHaul = pathname?.startsWith("/earthhaul");
  const isGateway = pathname === "/";
  const isFlex = !isHaul && !isGateway;

  useEffect(() => {
    if (typeof document !== "undefined") {
      const targetElement = document.documentElement;
      if (isFlex) {
        targetElement.classList.add("theme-earthflex");
      } else {
        targetElement.classList.remove("theme-earthflex");
      }
    }
  }, [pathname, isFlex]);

  const brandTextTitle1 = "EARTH";
  const brandTextTitle2 = isHaul ? "HAUL" : isGateway ? "INDIA" : "FLEX";
  const brandSub = isHaul ? "Logistics" : isGateway ? "Group" : "Industries";
  const brandDesc = isHaul 
    ? "Industrial Bulk Supply-Chain Logistics" 
    : isGateway 
      ? "Engineering & Logistics Divisions" 
      : "Premium Industrial Belt Solutions";
  const brandColor = isHaul ? "#eab308" : isGateway ? "#1B70B3" : "#0B8A1E";
  const brandColorBg = isHaul ? "bg-yellow-500/5" : isGateway ? "bg-primary/5" : "bg-accent/5";
  const brandTextAccent = isHaul ? "text-yellow-500" : isGateway ? "text-primary" : "text-accent";
    ...
    <AnimatePresence>
      {isVisible && (
        ...
        >
          ...
          <div className={`absolute w-[400px] h-[400px] ${brandColorBg} rounded-full blur-[100px] pointer-events-none`} />
          ...
          <div className="relative flex flex-col items-center gap-3">
            ...
            <div className="w-60 h-24 relative">
              <svg viewBox="0 18 160 64" className="w-full h-full text-white">
                ...
                <g className="spin-left">
                  ...
                  <circle cx="40" cy="50" r="4.5" fill={brandColor} />
                </g>
                ...
                <g className="spin-right">
                  ...
                  <circle cx="120" cy="50" r="4.5" fill={brandColor} />
                </g>
                ...
                {/* 1. Conveyor Belt / Fleet (Product) */}
                <g>
                  <circle cx="0" cy="0" r="7" fill="#0c1a2e" stroke={brandColor} strokeWidth="1" />
                ...
                {/* 4. Logistics Arrows / Movement (Industry) */}
                <g>
                  <circle cx="0" cy="0" r="7" fill="#0c1a2e" stroke={brandColor} strokeWidth="1" />
                  <g transform="scale(0.42) translate(-11.5, -11.5)">
            ...
            <div className="text-center space-y-2.5 mt-2">
              <motion.h1 
                ...
                className="text-white text-2xl md:text-3xl font-black uppercase tracking-[0.25em] pl-[0.25em] font-sans animate-pulse"
              >
                <span>{brandTextTitle1}</span> <span className={brandTextAccent}>{brandTextTitle2}</span>
              </motion.h1>
              ...
              <motion.div 
                ...
                className={`text-[10px] font-mono font-bold tracking-[0.35em] ${brandTextAccent} uppercase pl-[0.35em]`}
              >
                {brandSub}
              </motion.div>
